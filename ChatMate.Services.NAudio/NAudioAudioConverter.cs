...
public class NAudioAudioConverter : IAudioConverter
{
    ...
    public string SelectContentType(string[] acceptedContentTypes, string generatedContentType)
    {
        if (acceptedContentTypes.Contains(generatedContentType))
            return ContentType = generatedContentType;
        ...
        return ContentType = common.Contains(generatedContentType) ? generatedContentType : common[0];
    }
    ...
    public async Task<AudioData> ConvertAudioAsync(AudioData input, CancellationToken cancellationToken)
    {
            ...
            try
            {
                ...
                switch (ContentType)
                {
                    case "audio/mpeg":
                        ...
                        return new AudioData(ms, ContentType);
                    case "audio/wav":
                    case "audio/x-wav":
        ...
        if(input.ContentType == "audio/mpeg" && ContentType is "audio/wav" or "audio/x-wav")
        {
        ...
        throw new NotSupportedException($"Input {input.ContentType} and output {ContentType} pair is not supported");
    }
}